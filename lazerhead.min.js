...
var jumping=!1,charging=!1,playerDied=!1,gameWon=!1,sin=Math.sin;function square(a){return 0>sin(a)?-1:1}function saw(a){a/=2*Math.PI;return 2*(a-Math.floor(a))-1}var actx=new webkitAudioContext,jsnode=actx.createScriptProcessor(512,0,1),t=0;jsnode.onaudioprocess=function(a){a=a.outputBuffer.getChannelData(0);for(var b=0;b<a.length;b++)a[b]=lazerSFX(t),t+=0.05};jsnode.connect(actx.destination);var lazerFiring;function lazerSFX(a){if(lazerFiring)return(sin(a)+sin(1.02*a))/2}
...
function updateGameState(){if(playerDied)return!1;lazerFiring=!1;playerAnim.name="idle";charging=!1;!0==pressed[76]&&(player.power<player.maxpower?(console.log("Aaaaaaaaaaa!!!"),player.power+=0.5):console.log("MAX POWER!"),charging=!0,effects.push(new risingLineEffect));!0!=pressed[65]||charging||(player.x-=3,playerAnim.name="running");!0!=pressed[68]||charging||(player.x+=3,playerAnim.name="running");!0!=pressed[32]||charging?jumping=!1:!jumping&&0<player.getBottom()&&(player.dy-=20,jumping=!0);
!0!=pressed[74]||charging||(lazerFiring=!0,effects.push(new lazerEffect(player.x+player.w-10,player.y+20)),player.getBottom()<consts.ground?player.x-=0.0010*Math.pow(player.power,2):player.x-=3.25E-4*Math.pow(player.power,2),10<player.power&&(player.power-=0.1));if(!0!=pressed[74]||20>player.power)player.y+=player.dy,player.dy+=1;camera=player.x-200;99<rand(0,100)&&effects.push(new lavaBallEffect);updateWalls();player.updateStatus();0<player.health&&Math.random()>(player.health/(player.w*player.h)||
0)&&effects.push(new hurtEffect)}function updateWalls(){walls.forEach(function(a){a.updateStatus()})}function resetGame(){effects=[];gameWon?effects.push(new superTextEffect("Welcome back champion!!!",100,0,300)):effects.push(new superTextEffect("Welcome back!",100,0,300));gameWon=playerDied=!1;player.x=0;player.y=300;player.power=consts.initialPower;player.health=player.h*player.w;walls.forEach(function(a){a.health=a.h*a.w})}
