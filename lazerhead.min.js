...
var jumping=!1,charging=!1,playerDied=!1,gameWon=!1,sin=Math.sin;function square(a){return 0>sin(a)?-1:1}function saw(a){a/=2*Math.PI;return 2*(a-Math.floor(a))-1}function tone(a,b){return sin(2*a*Math.PI*b)}var actx=new webkitAudioContext,jsnode=actx.createScriptProcessor(512,0,1),t=0;jsnode.onaudioprocess=function(a){a=a.outputBuffer.getChannelData(0);for(var b=0;b<a.length;b++)out=(lazerSFX(t)+playerInjuredSFX(t)+chargeSFX(t))/3,a[b]=out,t+=1/44E3};jsnode.connect(actx.destination);var lazerFiring;
function lazerSFX(a){return lazerFiring?(tone(a,261.63)+tone(1.03*a,261.63))/2:0}var playerInjured;function playerInjuredSFX(a){return 0<playerInjured?Math.random(a)+sin(520*a*Math.PI):0}var charget=0.15,chargeUp=!1;
function chargeSFX(a){return charging?(0.3<charget&&(chargeUp=!1),0.01>charget&&(chargeUp=!0),charget=chargeUp?charget+1/(44E3*(1.1-player.power/player.maxpower)):charget-1/(44E3*(1.1-player.power/player.maxpower)),a=260*(1+charget),(sin(2*charget*Math.PI*a)+sin(2.4*charget*Math.PI*a)+sin(3.4*charget*Math.PI*a))/3):0}
for(var walls=[],nextWall=300,i=0;i<consts.wallCount;i++){var height=rand(250,400),width=rand(50+5*i,100+5*i);walls.push(new Box(nextWall,consts.ground-height,width,height));var dist=rand(100,500),nextWall=nextWall+(width+dist)}walls.push(new Box(nextWall-20,consts.ground-450,300,450));var magicDoor=new Box(nextWall+50,consts.ground-126,50,126);
...
function updateGameState(){if(playerDied)return!1;lazerFiring=!1;playerInjured=0<playerInjured?playerInjured-1:0;playerAnim.name="idle";charging=!1;!0==pressed[76]&&(player.power<player.maxpower?(console.log("Aaaaaaaaaaa!!!"),player.power+=0.5):console.log("MAX POWER!"),charging=!0,effects.push(new risingLineEffect));!0!=pressed[65]||charging||(player.x-=3,playerAnim.name="running");!0!=pressed[68]||charging||(player.x+=3,playerAnim.name="running");!0!=pressed[32]||charging?jumping=!1:!jumping&&0<
player.getBottom()&&(player.dy-=20,jumping=!0);!0!=pressed[74]||charging||(lazerFiring=!0,effects.push(new lazerEffect(player.x+player.w-10,player.y+20)),player.getBottom()<consts.ground?player.x-=0.0010*Math.pow(player.power,2):player.x-=3.25E-4*Math.pow(player.power,2),10<player.power&&(player.power-=0.1));if(!0!=pressed[74]||20>player.power)player.y+=player.dy,player.dy+=1;camera=player.x-200;99<rand(0,100)&&effects.push(new lavaBallEffect);updateWalls();player.updateStatus();0<player.health&&
