window.requestAnimFrame=function(){return window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||function(a){window.setTimeout(a,1E3/60)}}();function rand(a,b){return a+Math.random()*(b-a)}function randColor(){return"#"+Math.floor(16777215*Math.random()).toString(16)}
var consts={ground:500,playerW:50,playerH:126,initialPower:25,wallCount:7,starsCount:250},effects=[],camera=0,ctx=document.getElementById("canvas").getContext("2d"),pressed={},player=new Box(0,300,consts.playerW,consts.playerH),playerAnim={},sprite=new Image;sprite.src="sprites.png";
for(var jumping=!1,charging=!1,playerDied=!1,gameWon=!1,walls=[],nextWall=300,i=0;i<consts.wallCount;i++){var height=rand(250,400),width=rand(50+5*i,100+5*i);walls.push(new Box(nextWall,consts.ground-height,width,height));var dist=rand(100,500),nextWall=nextWall+(width+dist)}walls.push(new Box(nextWall-20,consts.ground-450,300,450));for(var magicDoor=new Box(nextWall+50,consts.ground-126,50,126),skyline1=[],i=0;200>i;i++)skyline1[i]=rand(20,250);
for(var skyline2=[],i=0;100>i;i++)skyline2[i]=rand(50,200);for(var skyline3=[],skyline3dists=[],skyline3widths=[],i=0;100>i;i++)skyline3[i]=rand(250,400),skyline3dists[i]=rand(250,500),skyline3widths[i]=rand(50,100);for(var starxs=[],starys=[],starrads=[],i=0;i<consts.starsCount;i++)starxs.push(rand(-50,1E3)),starys.push(rand(0,consts.ground)),starrads.push(rand(1,4));
...
function drawBackground(a){a.fillStyle="#111122";a.fillRect(0,0,800,600)}function drawGround(a){a.fillStyle="#555555";a.fillRect(0,500,800,100)}function drawMagicDoor(a){a.save();a.fillStyle=randColor();magicDoor.fillRect(a);a.restore()}
...
function draw(){drawBackground(ctx);drawStarfield(ctx);drawSkyline1(ctx);drawSkyline2(ctx);drawSkyline3(ctx);drawGround(ctx);ctx.save();ctx.translate(-camera,0);drawMagicDoor(ctx);drawPlayer(ctx);drawWalls(ctx);drawEffects(ctx);ctx.restore()}function refresh(){}
...
(a.save(),a.fillStyle=this.color,a.globalAlpha=rand(0.2,0.1),a.beginPath(),a.arc(this.x+this.radius(),this.y,this.radius(),0,2*Math.PI,!0),a.closePath(),a.fill(),a.restore())};this.isAlive=function(){return this.age<this.maxage}}function cleanUp(){effects=effects.filter(function(a){return a.isAlive()})}setup();effects.push(new superTextEffect("Welcome to Lazerhead!!!",100,0,300));window.requestAnimFrame(gameLoop);
