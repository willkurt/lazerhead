var consts={ground:500,playerW:50,playerH:126,initialPower:25,wallCount:7,starsCount:250,playerInjuredSFXt:5,wallDamagedSFXt:1};var sin=Math.sin;function square(a){return 0>sin(a)?-1:1}function saw(a){a/=2*Math.PI;return 2*(a-Math.floor(a))-1}function tone(a,b){return sin(2*a*Math.PI*b)}var actx=new webkitAudioContext,jsnode=actx.createScriptProcessor(512,0,1),t=0;jsnode.onaudioprocess=function(a){a=a.outputBuffer.getChannelData(0);for(var b=0;b<a.length;b++)out=(lazerSFX(t)+playerInjuredSFX(t)+chargeSFX(t)+wallDamagedSFX(t))/4,a[b]=out,t+=1/44E3};jsnode.connect(actx.destination);var lazerFiring;
function lazerSFX(a){return lazerFiring?(tone(a,261.63)+tone(1.03*a,261.63))/2:0}var playerInjured;function playerInjuredSFX(a){return 0<playerInjured?Math.random(a)+sin(520*a*Math.PI):0}var wallDamaged;function wallDamagedSFX(a){return 0<wallDamaged?tone(a,120):0}var charget=0.15,chargeUp=!1;
...
function updateGameState(){if(playerDied)return!1;lazerFiring=!1;playerInjured=0<playerInjured?playerInjured-1:0;wallDamaged=0<wallDamaged?wallDamaged-1:0;playerAnim.name="idle";charging=!1;!0==pressed[76]&&(player.power<player.maxpower?(console.log("Aaaaaaaaaaa!!!"),player.power+=0.5):console.log("MAX POWER!"),charging=!0,effects.push(new risingLineEffect));!0!=pressed[65]||charging||(player.x-=3,playerAnim.name="running");!0!=pressed[68]||charging||(player.x+=3,playerAnim.name="running");!0!=pressed[32]||
charging?jumping=!1:!jumping&&0<player.getBottom()&&(player.dy-=20,jumping=!0);!0!=pressed[74]||charging||(lazerFiring=!0,effects.push(new lazerEffect(player.x+player.w-10,player.y+20)),player.getBottom()<consts.ground?player.x-=0.001*Math.pow(player.power,2):player.x-=3.25E-4*Math.pow(player.power,2),10<player.power&&(player.power-=0.1));if(!0!=pressed[74]||20>player.power)player.y+=player.dy,player.dy+=1;camera=player.x-200;99<rand(0,100)&&effects.push(new lavaBallEffect);updateWalls();player.updateStatus();
